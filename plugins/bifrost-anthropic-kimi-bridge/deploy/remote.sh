...
ssh "${SSH_OPTS[@]}" "${REMOTE_HOST}" "\
  docker logs '${CONTAINER_NAME}' 2>&1 | grep -E '${PLUGIN_NAME}|loading custom plugin|plugin status' | tail -n 40 && \
  printf '\n--- plugin rows ---\n' && \
...
if [[ -n "${VK:-}" ]]; then
  ...
  echo
  echo "[optional] runtime test with VK"
  ssh "${SSH_OPTS[@]}" "${REMOTE_HOST}" "curl -fsS -X POST 'http://127.0.0.1:${HOST_PORT}/anthropic/v1/messages' \
    -H 'content-type: application/json' \
    -H 'authorization: Bearer ${VK}' \
    -H 'anthropic-version: 2023-06-01' \
    -d '{\"model\":\"${MODEL}\",\"max_tokens\":80,\"messages\":[{\"role\":\"user\",\"content\":\"reply with exactly ok\"}]}'"
fi
